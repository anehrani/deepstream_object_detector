...
__global__ void decodeTensorYoloECuda(NvDsInferParseObjectInfo *binfo, float *boxes, float *classes,
                                      int outputSize, int netW, int netH, float minPreclusterThreshold)
{
  int x_id = blockIdx.x * blockDim.x + threadIdx.x;
  if (x_id >= outputSize)
    return;
  float maxProb = 0; // scores[x_id];
  int maxIndex = 0;  //(int) classes[x_id];
  for (int i = 0; i < 80; i++)
  {
    if (classes[x_id * 80 + i] > maxProb)
    {
      maxProb = classes[x_id * 80 + i];
      maxIndex = i;
    }
  }
                                          ...
                                          std::vector<NvDsInferParseObjectInfo> &objectList)
{
  ...
  decodeTensorYoloECuda<<<number_of_blocks, threads_per_block>>>(
      thrust::raw_pointer_cast(objects.data()), (float *)(boxes.buffer), (float *)(classes.buffer),
      outputSize, networkInfo.width, networkInfo.height, minPreclusterThreshold);
